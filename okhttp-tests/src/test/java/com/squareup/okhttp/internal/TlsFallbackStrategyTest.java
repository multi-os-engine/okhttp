...
public class TlsFallbackStrategyTest {
  ...
  public void allFallbacksSupported() throws Exception {
    ...
    try {
      fallbackStrategy.configureSecureSocket(supportsSslV3, "host", platform);
        ...
        assertContains(supportsSslV3.getEnabledProtocols(), TLSV1_AND_SSLV3);
    ...
    try {
      fallbackStrategy.configureSecureSocket(supportsSslV3, "host", platform);
      assertContains(supportsSslV3.getEnabledProtocols(), "SSLv3");
  ...
  public void sslV3NotSupported() throws Exception {
    ...
    try {
      ...
      assertContains(socket.getEnabledProtocols(), TLSV1_ONLY);
  ...
  private static void assertContains(String[] values, String... required) {
    Set<String> actual = new HashSet<String>(Arrays.asList(values));
    Set<String> expectedSubset = new HashSet<String>(Arrays.asList(required));
    assertTrue(actual + " must contain all of " + expectedSubset,
        actual.containsAll(expectedSubset));
  }
}
